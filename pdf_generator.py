...
def generate_pdf(student_name, student_roll, date_str, report_data):
    ...
    safe_roll = student_roll.replace("/", "-").replace(" ", "_")
    ...
    S_CONF  = _s("conf",  fontName="Helvetica-Oblique", fontSize=7,  textColor=colors.HexColor("#aaaaaa"), alignment=TA_CENTER, spaceAfter=10)
    ...
    S_FOOT  = _s("foot", fontName="Helvetica",          fontSize=7,  textColor=colors.HexColor("#888888"), alignment=TA_CENTER)

    def th(txt, fs=8.5):
        return Paragraph(txt, _s(f"th{txt[:3]}", fontName="Helvetica-Bold",
                                  fontSize=fs, textColor=WHITE, alignment=TA_CENTER))

    def td(txt, bold=False, color=BLACK, align=TA_LEFT, fs=8.5):
        return Paragraph(str(txt), _s(f"td{txt[:3]}", fontName="Helvetica-Bold" if bold else "Helvetica",
    ...
    det_tbl = Table(detail_data, colWidths=[W*0.22, W*0.28, W*0.22, W*0.28])
    ...
    for i, sec in enumerate(SECTIONS):
        ...
        for ev in evaluators:
            ...
            row.append(td("—" if val is None else str(val), align=TA_CENTER,
    ...
    # Total row
    tot_style = _s("tots", fontName="Helvetica-Bold", fontSize=9, textColor=WHITE)
    tot_row   = [Paragraph("TOTAL  (Max: 50)", tot_style)]
    for ev in evaluators:
    ...
    story.append(Paragraph(
        "Book Your Next Session: http://t.me/pg_appointment_bot", S_LINK
    ))
    story.append(Spacer(1, 12))

    # ── Remarks ───────────────────────────────────────────────────
    rem_hdr = Table([[Paragraph("Evaluator Remarks",
    ...
    rem_rows = [[Paragraph(ev["iv_name"], S_RH),
                 Paragraph(f'"{ev["remark"]}"', S_REM)] for ev in evaluators]
